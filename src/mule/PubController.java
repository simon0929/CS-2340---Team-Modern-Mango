...
public class PubController {
    ...
    @FXML
    private void handlePub(ActionEvent event) throws IOException {
    ...
    @FXML
    private void gamble(ActionEvent event) throws IOException {
        Random rand = new Random();

        int moneyBonus = calRoundBonus() * rand.nextInt(calTimeBonus());
        if (moneyBonus > 250) {
            GameController.currentPlayer.setMoney(GameController.currentPlayer.getMoney() + 250);
        } else {
    ...
    private int calRoundBonus() {
        ...
        if (GameController.roundNumber < 8) {
            return 100;
        }

        if (GameController.roundNumber < 12) {
            return 150;
        }
